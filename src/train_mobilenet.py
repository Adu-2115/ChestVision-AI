"""
Training script for the MobileNetV2 multimodal model.
Identical training loop to train.py — only the model import and save dir differ,
so the AUC comparison between architectures is fair (same data, same hyperparameters).

Save this as: src/train_mobilenet.py  (same level as train.py)
"""
import os
import sys
import torch
...
from src.models.densenet import get_class_weights  # shared utility, works for any model


# ── Config ────────────────────────────────────────────────
DATASET_ROOT = r'D:\Datasets\CheXpert-v1.0-small'
SAVE_DIR     = r'D:\Projects\ChestVision-AI\checkpoints_mobilenet'
BATCH_SIZE   = 16
NUM_EPOCHS   = 20
LR           = 1e-4
IMG_SIZE     = 224
...
def plot_history(history, save_dir):
    ...
    axes[1].set_title('Mean AUC'); axes[1].set_xlabel('Epoch'); axes[1].legend()
    plt.tight_layout()
...
def train():
    ...
    # NOTE: MobileNetV2 is smaller and prone to overfitting faster than
    # EfficientNet-B0 was — watch epochs 5-8 closely, same as before.
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        train_loss, train_auc, _ = train_one_epoch(model, train_loader, criterion, optimizer, scaler, device)
        val_loss, val_auc, val_aucs = validate(model, val_loader, criterion, device)
        scheduler.step()
        ...
        if val_auc > best_auc:
            ...
            torch.save({
                ...
                'model_type':           'mobilenet_v2_multimodal',
